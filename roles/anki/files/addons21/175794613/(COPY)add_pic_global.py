...
def add_pic_global_friend_group(self:"start_main", tab_widget:"QTableWidget", icon_type="hexagon", board_type="Today"):
    ...
    # icon_item.setToolTip("icon")
    # tab_widget.setHorizontalHeaderItem(new_column_index + 2, icon_item) # start 1


    # for column_index in range(tab_widget.columnCount()):
    # for column_index in range(2, tab_widget.columnCount()):
    #     tab_widget.horizontalHeader().setSectionResizeMode(column_index, QHeaderView.ResizeMode.Interactive)
    ...
    for i in range(tab_widget.rowCount()):
        ...
        for league_lists in self.response[1]:
            if item == league_lists[0]:
                ...
                # sort_league_name = sort_league_names.get(league_name, "04_Delta")
                # sort_league_name += f" {rank_a_f}"
                qtab_le.setText(f" {rank_a_f} ")
                # qtab_le.setText(sort_league_name)
                if not self.config.get("gamification_mode", True):
                ...
                # if True:
                #     user_icons_downloader.get_icon_path_from_username(
                #         tab_widget, i, item, TOOLTIP_ICON_SIZE)

                # user_icons_downloader.add_make_path_and_icons_list(
    ...
    # is_mini_text = config.get("mini_text", True)
    # tab_widget.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Interactive)
    ...
    #         tab_widget.horizontalHeader().setSectionResizeMode(column_index, QHeaderView.ResizeMode.Interactive)
    #         actual_width = tab_widget.columnWidth(column_index)
    ...
    # tab_widget.setColumnWidth(8, 15)
    # tab_widget.horizontalHeader().setSectionResizeMode(8, QHeaderView.ResizeMode.Interactive)

    # tab_widget.setColumnWidth(9, 15)
    # tab_widget.horizontalHeader().setSectionResizeMode(9, QHeaderView.ResizeMode.Interactive)
    ...
    ### font and icons size ####
    # local_image_path, user_icon = user_icons_downloader.deobfuscate_image(local_image_path)
    # user_file_icon_path = f'<img src="{local_image_path}" width="{TOOLTIP_ICON_SIZE}" height="{TOOLTIP_ICON_SIZE}"><br>'
    # # .setIcon(country_icon)
    # if not user_icon is None:
    #     user_name_item = tab_widget.item(i, 1)
    #     user_name_item.setIcon(user_icon)
    #     # tab_widget.setRowHeight(i, 34)

    #     # font = user_name_item.font()
    #     # font.setPointSize(14)
    #     # user_name_item.setFont(font)
    #     # icon_size = 22
    #     # tab_widget.setIconSize(QSize(icon_size, icon_size))
