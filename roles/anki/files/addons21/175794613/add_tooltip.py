...
def set_pic_league_tab(self:"start_main"): # not used yet
    ...
    {"gold": 0, "silver": 0, "bronze": 0, "results": {"leagues": ["Delta", "Delta", "Gamma", "Gamma", "Gamma"], "seasons": [1, 2, 3, 4, 5], "xp": [139909, 2541037, 1691500, 1374747, 511386], "rank": [175, 21, 31, 71, 180]}}

    history = json.loads(item[6])
    history["results"]

    latest_league = history.get("results", {}).get("leagues", [None])[-1]
    latest_season = history.get("results", {}).get("seasons", [None])[-1]
    latest_xp = history.get("results", {}).get("xp", [None])[-1]
    latest_rank = history.get("results", {}).get("rank", [None])[-1]
